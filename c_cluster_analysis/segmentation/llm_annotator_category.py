...
RESPONSE_SCHEMA = """{
  "annotations": [
    {
      "sentence_id": int,          # 1-based index in the list below
      "categories": List[str],     # one (max two) from CATEGORY_DEFINITIONS
      "other_label": str | null    # short label only if category == "other"
    }
  ]
}
"""
...
def _build_prompt(question: str, sentences: Sequence[str]) -> str:
    ...
    return (
        ...
        "category, only if two fit equally well, **maximum two**, from the list below to **each** sentence.\n\n"
        f"{CATEGORY_DEFINITIONS}\n\n"
        "Input question (context, do **NOT** label):\n"
        f"{{{question}}}\n\n"
        "Sentences to be categorized:\n"
        f"{numbered}\n\n"
