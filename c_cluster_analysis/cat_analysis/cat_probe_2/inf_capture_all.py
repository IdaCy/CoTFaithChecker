...
def run_probe_steer_inference(
    ...
    for sid, sent in enumerate(sentences, 1):
        ...
        # (hidden_states is a tuple: layer0 … layerN)
        mean_hs = _mean_hidden_states(out.hidden_states, len(sent_ids))  # (L,D)
        mean_hs = mean_hs.detach().to("cpu").half()     # free VRAM early

        out_rows.append(dict(
            sentence_id = sid,
            sentence    = sent,
            mean_hidden_state = mean_hs.tolist()        # JSON-serialisable
        ))
