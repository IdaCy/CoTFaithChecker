...
accelerator = Accelerator()                  # creates one process per GPU
# <<< accelerate

# >>> progress ────────────────────────────────────────────────────────────────
RANK     = accelerator.local_process_index
IS_MAIN  = accelerator.is_main_process
logging.info("[rank%d] process launched", RANK)
# <<< progress ────────────────────────────────────────────────────────────────


def _device():                       # unchanged public API
    # >>> accelerate
    return accelerator.device        # each process sees the right CUDA device
    # <<< accelerate
...
def load_model_and_tokenizer(model_path: str):
    ...
    # >>> accelerate
    # Wraps the model for distributed inference *and* moves it to the
    # correct GPU for this process.
    model = accelerator.prepare(model)
    # keep a convenient attribute for downstream calls that expect `.device`
...
def run_sentence_level_inference(
    ...
    input_ids = torch.tensor([tok_prefix + tok_q + tok_hint + tok_suffix], device=device)
        ...
        for _ in range(max_new_tokens):
            ...
            while n_completed < completed_cnt:
                ...
                seq_guess = (
                    ...
                    + tok_suffix
                    + generated
                    + tok.encode(intervention_prompt, add_special_tokens=False)
                )
                guess_ids = torch.tensor([seq_guess], device=device)
                g_logits = model(input_ids=guess_ids).logits[:, -1, :][0]
                ans_logits = {lbl: float(g_logits[tid].item()) for lbl, tid in opt_ids.items()}
...
def run_batch_from_files(
    ...
    **generation_kwargs,
    ...
    if whitelist_file and Path(whitelist_file).exists():
        ...
        logging.info("filtered %d questions from %d", len(questions), len(questions))
    ...
    if hints_file and Path(hints_file).exists():
        hints_map = {
            h["question_id"]: h
            for h in json.loads(Path(hints_file).read_text(encoding="utf-8"))
        }

    # >>> accelerate
    # split the workload so each GPU/process handles a different slice
    questions = accelerator.split_between_processes(questions)
    # <<< accelerate

    # >>> progress ────────────────────────────────────────────────────────────
    logging.info(
        "[rank%d] received %d questions for processing",
        RANK, len(questions)
    )
    # <<< progress ────────────────────────────────────────────────────────────
    ...
    for q in questions:
        # >>> progress
        _idx += 1
        if _idx == 1 or _idx % 10 == 0 or _idx == len(questions):
            logging.info(
                "[rank%d] progress %d/%d (question_id=%s)",
                RANK, _idx, len(questions), q["question_id"]
            )
        # <<< progress
    ...
    # >>> accelerate
    # gather all partial result lists onto the main process
    if dist.is_initialized():
        gathered: List[List[dict]] = [None] * dist.get_world_size()
        dist.all_gather_object(gathered, results)
        if accelerator.is_main_process:
            results_all = [item for sub in gathered for item in sub]
        else:
            results_all = None
    ...
    if accelerator.is_main_process and results_all is not None:
        Path(output_file).write_text(json.dumps(results_all, indent=2), encoding="utf-8")
        logging.info("saved %d records to %s", len(results_all), output_file)
    # <<< accelerate

    return results  # local results for the worker; the file has full results
